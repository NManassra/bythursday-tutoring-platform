...
export async function PUT(req:Request,{params}:{params:Promise<{attemptId:string}>}){
  ...
  let body;
  try{body=submitSchema.parse(await req.json())}catch{return NextResponse.json({error:"Invalid answers"},{status:400})}
  ...
  for(const answer of body.answers){
    ...
    if(answer.optionId&&!q.options.some(o=>o.id===answer.optionId)){
