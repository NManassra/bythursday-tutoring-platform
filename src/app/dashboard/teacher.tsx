 ...
 return <main className="shell" dir="auto">
   ...
   <section className="card" style={{marginTop:20}}>
     ...
     {q.map(x=><article className="quiz-row" key={x.id}>
       ...
       {analytics[x.id]&&(()=>{const current=analytics[x.id]!;return <>
       <div className="analytics-grid">
         <div><span className="muted">Attempted</span><strong>{current.attempted}/{current.assigned}</strong></div>
         <div><span className="muted">Average</span><strong>{current.average}</strong></div>
         <div><span className="muted">Highest</span><strong>{current.highest}</strong></div>
         <div><span className="muted">Lowest</span><strong>{current.lowest}</strong></div>
         <div><span className="muted">Completion</span><strong>{current.completionRate}%</strong></div>
       </div>
       {!!current.questionPerformance?.length&&<div className="question-analytics" aria-label="Question performance">
         <h4>Question performance</h4>
         {current.questionPerformance!.map((item,index)=><div className="question-analytics-row" key={item.questionId}>
