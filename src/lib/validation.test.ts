...
describe("validation",()=>{
  ...
  it("accepts valid quiz input",()=>expect(quizSchema.parse({title:"Quiz",opensAt:new Date(),closesAt:new Date(Date.now()+10000),durationMinutes:10,pointsPerQuestion:1,negativeMarkPercent:0,classId:"c",questions:[{text:"Q",options:[{text:"A",isCorrect:true},{text:"B",isCorrect:false}]}]}).title).toBe("Quiz"));
});